  ...
  return (
    <div className="min-h-screen bg-[#faf8f3] text-[#111111]" style={{ fontFamily: 'var(--font-jost), sans-serif' }}>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 min-h-screen">
        <div className="flex flex-col justify-end px-6 lg:px-14 pt-32 pb-12 lg:pb-20 bg-[#faf8f3] order-2 lg:order-1">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }}>
            <Tag>{service.tag} · Panama City</Tag>
            <h1 className="font-display font-light text-[#111111] mb-6" style={{ fontSize: 'clamp(2.6rem, 5vw, 5.5rem)', lineHeight: 1.02 }}>
              {service.title}
            </h1>
            <p className="font-light mb-10 max-w-lg" style={{ fontSize: '1.1rem', color: 'rgba(17,17,17,0.88)', lineHeight: 1.75 }}>
              {service.description}
            </p>
      ...
      {/* Manifesto */}
      <section className="bg-[#ede9df] px-6 lg:px-20 py-16 lg:py-24">
        <p className="font-display font-light italic text-center mx-auto max-w-3xl" style={{ fontSize: 'clamp(1.4rem, 3vw, 2.4rem)', lineHeight: 1.55, color: 'rgba(17,17,17,0.88)' }}>
...
function OtherServices({ otherServices, lang, sp }) {
  return (
    <section className="bg-[#ede9df] px-6 lg:px-16 py-14 lg:py-20">
      <div className="max-w-7xl mx-auto">
        <p className="text-[0.6rem] tracking-[0.3em] uppercase mb-8" style={{ color: 'rgba(17,17,17,0.25)' }}>{sp.otherServices}</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-white/5">
          {otherServices.map(svc => (
            <Link key={svc.key} href={`/${lang}/${svc.slug}`} className="flex flex-col gap-3 p-6 lg:p-8 bg-[#ede9df] no-underline">
